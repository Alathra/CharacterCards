...
public class HookManager implements Reloadable {
    ...
    public HookManager(CharacterCards plugin) {
        this.plugin = plugin;
    }
    ...
    public void onLoad(CharacterCards plugin) {
        for (Hook hook : Hook.values()) {
            try {
                ...
                final AbstractHook hookInstance = hook.getHookClass().getDeclaredConstructor(CharacterCards.class).newInstance(plugin);
